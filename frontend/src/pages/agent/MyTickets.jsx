...
import { FiSearch, FiEye, FiAlertTriangle } from 'react-icons/fi';
...
const MyTickets = () => {
  ...
  useEffect(() => { load(); }, [filters.status, filters.priority, filters.stage]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">Tickets currently assigned to you.</p>
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <div className="relative w-full max-w-xs">
          <FiSearch className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" size={16} />
          <input className="input pl-10" placeholder="Search tickets" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} />
        </div>
      ...
      {!tickets ? (
        ...
        <EmptyState icon="🎯" title="No tickets assigned to you yet" subtitle="New assignments will show up here automatically." />
      ) : (
        <DataTable columns={['Ticket', 'Client', 'Category', 'Priority', 'Status', '']}>
          {tickets.map((t) => (
            <tr key={t._id} className="hover:bg-paper/50">
              ...
              <td className="px-5 py-3.5">
                <Link to={`/agent/tickets/${t._id}`} className="rounded-lg p-1.5 text-muted hover:bg-brand-50 hover:text-brand-600">
                  <FiEye size={16} />
                </Link>
              </td>
            </tr>
